...
- **性能面板（默认关）**：Styler「性能」组勾选「记录性能数据」后，纯内存记录最近 300 次查询（耗时/命中/分段），实时显示 索引加载 · JS堆 · P50/P95/max + 阶段均值（扫描/排序/正文匹配）；「跑分基准」跑 6 个代表词 × 20 遍出表（P95>16ms 标红）。关闭即清空样本，查询路径零额外开销；设置键 `perf`。searchTags 使用索引加载后预计算的小写匹配缓存 + 每查询一次性配置读取（约 2 倍提速，结果与旧实现逐项等价）
