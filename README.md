...
| 面板布局 `panelImg` | 开/关 | 关 | 关=文字优先：释义占满剩余高度独立滚动，示例图自动缩放封顶 `min(220px, 34vh)`，空间不足先挤图片；开=图片优先：图片按面板宽完整展示（封顶 `min(78vh, 900px)`，基本免滚动看全图），释义≤140px、整栏滚动。点击图片=弹出悬浮大图卡（×/Esc/点卡外关闭） |
